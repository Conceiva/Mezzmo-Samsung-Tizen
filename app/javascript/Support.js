...
Support.updateURLHistory = function(page,title,url,title2,url2,selectedItem,topLeftItem,isTop,playlistId,parentItem) {
	...
	if (this.previousPageDetails.length > 0) {
		...
		if (this.previousPageDetails[this.previousPageDetails.length-1][8] != playlistId) {
			...
			console.log ("Adding new item: " + this.previousPageDetails.length);
		} else {
			if (this.previousPageDetails[this.previousPageDetails.length-1][0] != page) {
				...
				console.log ("New Item not added - Is duplicate of previous page: " + this.previousPageDetails.length);
			}		
		}
	} else {
		this.previousPageDetails.push([page,title,url,title2,url2,selectedItem,topLeftItem,isTop,playlistId,parentItem]);
...
Support.removeLatestURL = function() {
	...
	console.log ("Removed item: " + this.previousPageDetails.length);
}
...
Support.processReturnURLHistory = function(bookmark) {
	...
	if (this.previousPageDetails.length > 0) {
		...
		console.log ("Just after removing item" + this.previousPageDetails.length);
	} else {
	...
	for (var index = startPos; index < endPos; index++) {
		if (isResume == true) {
			...
			//----------------------------------------------------------------------------------------------
				...
				console.log("Unhandled Item type: "+Items[index].type)
				var title = Items[index].title;		
				if (Items[index].poster) {		
					var imgsrc = Items[index].poster;
					var createdDate = Items[index].PremiereDate;
...
Support.processSelectedItem = function(page,ItemData,startParams,selectedItem,topLeftItem,isTop,genreType,isLatest) {	
	...
	console.log("Type: "+ItemData[selectedItem].type +" MediaType: "+ItemData[selectedItem].MediaType);
...
Support.playSelectedItem = function(page,ItemData,startParams,selectedItem,topLeftItem,isTop) {
	...
	console.log ("playSelectedItem: Type "+ItemData[selectedItem].type);
...
Support.initViewUrls = function() {
	console.log("Initialising View URL's for this user");
...
Support.getViewUrl = function(viewName) {
	console.log("returning url for "+viewName+" : "+this[viewName]);
...
Support.getLibraryItem = function (itemData, NumberReturned, TotalMatches) {
	if (itemData.length != 0) {
		console.log("LibraryItem: " + itemData[0].title + " Desc: " + itemData[0].Overview);
